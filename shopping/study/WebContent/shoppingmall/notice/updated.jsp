...
/* 		if(f.email.value){  //이메일의 value값을 가져와라 
		...
		<%-- f.action = "<%=cp %>/shoppingmall/notice/updated_ok.jsp?pageNum=<%=pageNum%><%=param%>"; --%>
		f.action = "<%=cp%>/shop/notice/updated_ok.do?pageNum=<%=pageNum%><%=param%>";
		f.submit();
					...
					<form action="" method="post" name="myForm">
								...
								<dl>
									...
									<dd>
										<input type="text" name="subject"
											value="<%=dto.getSubject()%>" size="60" maxlength="100"
											class="boxTF" />
									</dd>
								</dl>
							</div>
						...
						<div id="bbsCreated_footer">
							...
							<input type="button" value=" 수정하기 " class="btn2"
								onclick="sendIt();" /> <input type="button" value="수정취소"
								class="btn2"
